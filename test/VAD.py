...
class VoiceActivityDetector():
  ...
  def __init__(self, input_wav_file, file_name, s_freq):
    ...
    self.speech_energy_threshold = 0.30 # 30% of energy in voice band
  ...
  def _calculate_energy(self, data):
    ...
    rms_energy = np.sqrt(np.mean(np.array(data_amplitude)**2))
    return data_energy, rms_energy
  ...
  def _calculate_normalized_energy(self, data):
    ...
    data_energy, rms_energy = self._calculate_energy(data)
    energy_freq = self._connect_energy_with_frequencies(data_freq, data_energy)
    return energy_freq, rms_energy
  ...
  def _smooth_speech_detection(self, detected_windows):
    ...
    if median_window % 2 == 0:
      ...
      #print("Median energy is: ", median_energy)
  ...
  def convert_windows_to_readable_labels(self, audio, detected_windows):
    ...
    for index, window in enumerate(detected_windows):
      ...
      if(index == np.shape(detected_windows)[0]-1 and window[1]==1.0 and is_speech == 1):
        is_speech = 0
        speech_time_end = window[0] / self.rate
        speech_label['speech_end'] = speech_time_end
        speech_time.append(speech_label)
  ...
  def detect_speech(self):
    ...
    while(sample_start < (len(data)-sample_window)):
        # Select only the region of data in the window
        ...
        # Full energy
        energy_freq, rms_energy = self._calculate_normalized_energy(data_window)
        ...
        # The rms value of the energy in the considered data frame
        #print("The rms value: ", rms_energy)

        #print("Voice energy: ", sum_voice_energy)
        #print("Full energy: \n", sum_full_energy)

        if rms_energy < self.speech_energy_threshold:
        	speech_ratio = sum_voice_energy > rms_energy		# For very silent audios
        else:
        	speech_ratio = speech_ratio > self.speech_energy_threshold
        #print("Speech ratio now is: ", speech_ratio)
        ...
        #print("Its detected: ", detected_windows, '\n')
        detected_windows[:, 1] = self._smooth_speech_detection(detected_windows)
        #print("Dim 1 shape: ", np.shape(detected_windows[:, 1]))
...
def remove_silence(audio_file, audio_file_name, s_freq, data_path, session):
  ...
  """i = 0
  for win in raw_detection:
    if win[1] == 1.0:
      i = i+1
  print("Ones count is: ", i)"""
  new_speech = v.convert_windows_to_readable_labels(audio_file, raw_detection)

  #print("new speech: ", np.shape(new_speech))
