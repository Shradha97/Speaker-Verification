...
def read_files(audio_dir, path, session):
  if session == 'train':
    ...
    print("Reading files from the Inference Set...")
    print("Path: ", path, "\n")
    with open(path, 'r') as f:
      file_path = f.readlines()
...
def get_label(audio_file, session):
  ...
  if session == 'inference':    # Just for testing
    audio_parts[0] = audio_parts[0]+'_'+audio_parts[1]
...
def break_audio_frames(audio):
    feature_parts = np.matrix([]).reshape((0, cfg.nmfcc))
    if np.shape(audio)[1] < cfg.nmfcc:       # We need same number of frames and n_mels in our features
        while np.shape(audio)[1] < cfg.nmfcc:
            audio = np.concatenate((audio, audio), axis = 1)
    for i in range(0, np.shape(audio)[1], cfg.nmfcc):
        if i + cfg.nmfcc > np.shape(audio)[1]:              # If duing breaking number of remaining frames are less than nmfcc then attach the first few frames to compensate for the required number of frames
            end = (i + cfg.nmfcc) - np.shape(audio)[1]
            tmp = np.hstack((audio[:, i:np.shape(audio)[1]], audio[:, 0: end]))
            feature_parts = np.concatenate([feature_parts, tmp], axis = 0)
            break
        else:
            feature_parts = np.concatenate([feature_parts[:, 0:cfg.nmfcc], audio[:, i:i+cfg.nmfcc]], axis = 0)
    features_matrix = np.reshape(np.array(feature_parts), (-1, cfg.nmfcc, cfg.nframes))
...
def get_mfcc(audio, norm_audio, audio_file, speaker_label, s_freq, data_path, session):
  ...
  speech = np.abs(librosa.stft(audio_data, window = "hamming", n_fft = cfg.nfft, win_length = int(cfg.window*s_freq), hop_length = int(cfg.hop_train*s_freq)))**2
  speech = librosa.feature.melspectrogram(S = speech, y = audio_data, n_mels = cfg.nmels)
  feats = librosa.feature.mfcc(S = librosa.power_to_db(speech), n_mfcc = cfg.nmfcc)
...
def one_hot_speaker_label(index, depth = cfg.num_classes):  # Here 180 is the number of speakers in the train set
    a = tf.Variable(index)
...
def preprocess_data(audio_dir, set_path, data_path, session):
    ...
    for (audio, norm_audio, audio_file) in zip(audio_list, norm_audio_list, audio_file_list):
        if session  != 'inference':
            ...
            if speaker_label != prev_speaker:
                ...
                utterances_spec = np.empty((0, cfg.nmfcc, cfg.nframes))            # Resetting the list for the new speaker
    ...
    if session == 'train':
      ...
      save = open(os.path.join(os.path.join(audio_dir, data_path), "One_hot.pickle"), 'ab')    # Storing the corresponding speakers' one hot encodings in a pickle file
